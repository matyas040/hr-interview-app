...
/**
 * Generates a professional rejection email based on interview evaluation.
 */
export async function generateRejectionEmail(interview, evaluation) {
    const apiKey = getApiKey();
    if (!apiKey) throw new Error('NO_API_KEY');

    const lang = getLang();
    const prompt = lang === 'hu'
        ? `Te egy empatikus Senior HR specialista vagy. Írj egy kedves, profi elutasító e-mailt a következő adatok alapján:
JELÖLT: ${interview.candidateName}
MUNKAKÖR: ${interview.roleTitle || '(megnevezés)'}
ÖSSZEFOGLALÓ ÉRTÉKELÉS: ${evaluation.summary}
AJÁNLÁS: ${evaluation.recommendation}

KÖVETELMÉNYEK:
1. Legyen nagyon udvarias és bátorító.
2. Köszönd meg az idejét.
3. Említs meg 1-2 pozitívumot az értékelésből (ha van).
4. Indokold meg az elutasítást finoman, építő jelleggel az összefoglaló alapján.
5. VÁLASZOLJ MAGYARUL. 
6. Adj vissza egy JSON-t: { "subject": "Tárgy", "body": "Levél törzse\\n(használd az új sort)" }`
        : `You are an empathetic Senior HR specialist. Write a kind, professional rejection email based on the following:
CANDIDATE: ${interview.candidateName}
ROLE: ${interview.roleTitle || '(role)'}
EVALUATION SUMMARY: ${evaluation.summary}
RECOMMENDATION: ${evaluation.recommendation}

REQUIREMENTS:
1. Be very polite and encouraging.
2. Thank them for their time.
3. Mention 1-2 positives from the evaluation (if any).
4. Explain the rejection gently and constructively based on the summary.
5. ANSWER IN ENGLISH.
6. Return a JSON: { "subject": "Subject", "body": "Email body\\n(use new lines)" }`;
