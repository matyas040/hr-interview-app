export function renderRoleManager(container, params = {}) {
        ...
        container.innerHTML = `
            <div class="mb-6">
                ...
                <div class="flex justify-between items-center">
                    <h2 style="font-size: 1.5rem; font-weight: 600;">Munkakörök szerkesztése</h2>
                    <button class="btn btn-primary" id="btn-add-role">
                        <i data-lucide="plus"></i> Új munkakör
                    </button>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 2rem; align-items: flex-start;">
                ...
                <div class="card" style="flex: 0 0 300px; padding: 1rem;">
                    <h3 class="mb-4" style="font-size: 1rem; font-weight: 600; padding: 0 0.5rem;">Munkakörök</h3>
                    <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                        ${roles.map(r => `
                            ...
                            >
                                ...
                                <div style="font-size: 0.75rem; color: var(--text-secondary);">${r.questions.length} kérdés${r.pdfName ? ' · 📎 PDF' : ''}</div>
                            </button>
                        `).join('')}
                        ${roles.length === 0 ? '<p style="color: var(--text-secondary); padding: 1rem;">Nincs munkakör</p>' : ''}
                    </div>
                </div>
                ...
                <div style="flex: 1;">
                    ${activeRole ? `
                        <div class="card mb-4">
                            <div class="form-group mb-0 flex gap-2">
                                ...
                                <button class="btn btn-success" id="btn-save-role">Mentés</button>
                        ...
                        <div class="card mb-4">
                            <div class="flex justify-between items-center mb-4">
                                <h3 style="font-size: 1.125rem; font-weight: 500;">Kérdések (${activeRole.questions.length})</h3>
                            </div>
                                ...
                                ${activeRole.questions.map((q, index) => `
                                    <div style="display: flex; gap: 0.75rem; align-items: flex-start;">
                                        ...
                                        <select class="form-input q-type-select" data-qid="${q.id}" style="width: auto; min-width: 130px; font-size: 0.82rem; margin-top: 0;" title="Válasz típusa">
                                            <option value="short" ${(q.answerType || 'detailed') === 'short' ? 'selected' : ''}>⚡ Rövid válasz</option>
                                            <option value="detailed" ${(q.answerType || 'detailed') === 'detailed' ? 'selected' : ''}>📝 Részletes válasz</option>
                                        </select>
                                        <button class="btn btn-icon btn-danger-text btn-del-q" data-qid="${q.id}" style="margin-top: 0.25rem;"><i data-lucide="x"></i></button>
                                    </div>
                                `).join('')}
                                ${activeRole.questions.length === 0 ? '<p style="color: var(--text-secondary);">Még nincsenek kérdések. Adj hozzá egyet!</p>' : ''}
                            </div>

                            <div style="display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap;">
                                <input type="text" id="new-q-input" class="form-input" placeholder="Új kérdés szövege..." style="flex: 1; min-width: 200px;">
                                <select id="new-q-type" class="form-input" style="width: auto; min-width: 140px; font-size: 0.85rem;">
                                    <option value="detailed">📝 Részletes válasz</option>
                                    <option value="short">⚡ Rövid válasz</option>
                                </select>
                                <button class="btn btn-primary" id="btn-add-q"><i data-lucide="plus"></i> Hozzáadás</button>
                            </div>
                        </div>
                        ...
                        <div class="card mb-4">
                            <h3 style="font-size: 1.125rem; font-weight: 500; margin-bottom: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
                                <i data-lucide="brain-circuit" style="width: 1rem; color: #8b5cf6;"></i> Munkaköri leírás
                                <span style="font-size: 0.7rem; background: rgba(139,92,246,0.12); color: #8b5cf6; border: 1px solid rgba(139,92,246,0.3); border-radius: 1rem; padding: 0.15rem 0.55rem; font-weight: 700;">AI alapja</span>
                            </h3>
                            <p style="color: var(--text-secondary); font-size: 0.875rem; margin-bottom: 1.25rem;">
                                Az AI a kiértékelésnél figyelembe veszi ezt a dokumentumot. Töltsd fel a munkakör elvárásait (PDF vagy TXT).
                            </p>
                            ${activeRole.jdFileName ? `
                            <div style="display: flex; align-items: center; gap: 1rem; padding: 0.875rem 1rem; background: rgba(139,92,246,0.06); border: 1px solid rgba(139,92,246,0.2); border-radius: var(--radius-md); margin-bottom: 1rem;">
                                <i data-lucide="file-badge" style="width: 1.5rem; color: #8b5cf6; flex-shrink: 0;"></i>
                                <div style="flex: 1; min-width: 0;">
                                    <div style="font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${activeRole.jdFileName}</div>
                                    <div style="font-size: 0.75rem; color: var(--text-secondary);">Munkaköri leírás · AI elemzés alapja</div>
                                </div>
                            ...
                            <label class="btn btn-secondary" style="cursor: pointer; display: inline-flex; align-items: center; gap: 0.5rem;">
                                <i data-lucide="upload"></i> ${activeRole.jdFileName ? 'Fájl cseréje' : 'Fájl feltöltése (PDF / TXT)'}
                                <input type="file" accept=".pdf,.txt" id="jd-upload-input" style="display: none;">
                            </label>
                        ...
                        <div class="card">
                            <h3 style="font-size: 1.125rem; font-weight: 500; margin-bottom: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
                                <i data-lucide="file-text" style="width: 1rem; color: var(--accent);"></i> Olvasnivaló dokumentum (PDF)
                            </h3>
                            <p style="color: var(--text-secondary); font-size: 0.875rem; margin-bottom: 1.25rem;">
                                Ha csatolsz egy PDF-et, a jelölt a kérdőív megkezdése előtt el kell olvassa és megerősítse.
                            </p>
                            ${activeRole.pdfName ? `
                            <div style="display: flex; align-items: center; gap: 1rem; padding: 0.875rem 1rem; background: rgba(59,130,246,0.06); border: 1px solid rgba(59,130,246,0.2); border-radius: var(--radius-md); margin-bottom: 1rem;">
                                ...
                                <div style="flex: 1; min-width: 0;">
                                    ...
                                    <div style="font-size: 0.75rem; color: var(--text-secondary);">Csatolt dokumentum</div>
                                </div>
                                <button class="btn btn-secondary" id="btn-preview-pdf" style="font-size: 0.8rem; padding: 0.4rem 0.8rem;">
                                    <i data-lucide="eye"></i> Előnézet
                                </button>
                            ...
                            ` : ''}
                            <label class="btn btn-secondary" style="cursor: pointer; display: inline-flex; align-items: center; gap: 0.5rem;">
                                <i data-lucide="upload"></i> ${activeRole.pdfName ? 'PDF cseréje' : 'PDF feltöltése'}
                    ...
                    ` : `
                        <div class="card flex flex-col items-center justify-center text-center" style="min-height: 300px;">
                            ...
                            <h3 style="font-size: 1.25rem; font-weight: 500;" class="mb-2">Válassz munkakört</h3>
                            <p style="color: var(--text-secondary);">Vagy hozz létre egy újat a bal oldali gombbal.</p>
                        </div>
                    `}
                </div>
            </div>
    ...
    const attachEvents = (activeRole) => {
        ...
        document.getElementById('btn-add-role')?.addEventListener('click', () => {
            const newRole = window.appStore.addRole('Új Munkakör');
        ...
        if (activeRole) {
            ...
            document.getElementById('btn-delete-role')?.addEventListener('click', () => {
                if (confirm('Biztosan törölni szeretnéd ezt a munkakört? (A korábbi interjúk adatai megmaradnak, de új interjú nem indítható ezzel a munkakörrel)')) {
            ...
            // JD: file upload (PDF or TXT)
            document.getElementById('jd-upload-input')?.addEventListener('change', (e) => {
                const file = e.target.files[0];
                if (!file) return;
                const statusEl = document.getElementById('jd-upload-status');
                statusEl.innerHTML = `<span style="color: var(--text-secondary);">⏳ Feltöltés...</span>`;
                const reader = new FileReader();
                const isPdf = file.name.toLowerCase().endsWith('.pdf');
                reader.onload = () => {
                    if (isPdf) {
                        const base64 = reader.result.split(',')[1];
                        window.appStore.updateRoleJobDesc(activeRole.id, { jdPdfBase64: base64, jdFileName: file.name });
                    } else {
                        window.appStore.updateRoleJobDesc(activeRole.id, { jdText: reader.result, jdFileName: file.name });
                    }
                    statusEl.innerHTML = `<span style="color: var(--success);">✓ ${file.name} sikeresen feltöltve – az AI figyelembe veszi.</span>`;
                    setTimeout(() => render(), 800);
                };
                reader.onerror = () => {
                    statusEl.innerHTML = `<span style="color: var(--danger);">✗ Hiba a fájl olvasásakor.</span>`;
                };
            ...
            document.getElementById('btn-remove-jd')?.addEventListener('click', () => {
                if (confirm('Biztosan eltávolítod a munkaköri leírást?')) {
            ...
            document.getElementById('pdf-upload-input')?.addEventListener('change', (e) => {
                ...
                reader.onload = () => {
                    ...
                    statusEl.innerHTML = `<span style="color: var(--success);">✓ ${file.name} sikeresen csatolva.</span>`;
                    setTimeout(() => render(), 800);
                };
                reader.onerror = () => {
                    statusEl.innerHTML = `<span style="color: var(--danger);">✗ Hiba a fájl olvasásakor.</span>`;
            ...
            document.getElementById('btn-remove-pdf')?.addEventListener('click', () => {
                if (confirm('Biztosan eltávolítod a csatolt PDF-et?')) {
