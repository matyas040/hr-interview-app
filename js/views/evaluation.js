    ...
    return `
        <div class="card mb-6" style="padding: 2rem;" id="ai-score-card">
            <h3 style="font-size: 1.125rem; font-weight: 600; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
                <i data-lucide="brain-circuit" style="width: 1.125rem; color: var(--accent);"></i> AI Kiértékelés
            </h3>
            <div style="display: flex; align-items: center; gap: 1rem; padding: 1rem; background: var(--bg-primary); border-radius: var(--radius-md);">
                ...
                <div>
                    <div style="font-weight: 500;">Az AI elemzi a jelölt válaszait…</div>
                    <div style="font-size: 0.8rem; color: var(--text-secondary); margin-top: 0.2rem;">Ez néhány másodpercig tarthat.</div>
                </div>
            </div>
...
function renderAiResultCard(result, dateStr = null, interviewId = null) {
    ...
    const recColors = {
        'Erősen ajánlott':       { bg: 'rgba(16,185,129,0.1)', c: 'var(--success)' },
        'Ajánlott':               { bg: 'rgba(59,130,246,0.1)', c: '#3b82f6' },
        'Megfontolásra ajánlott': { bg: 'rgba(245,158,11,0.1)', c: 'var(--warning)' },
        'Nem ajánlott':           { bg: 'rgba(239,68,68,0.1)',  c: 'var(--danger)' }
    };
        ...
        <div class="card mb-6" style="padding: 2rem;" id="ai-score-card">
            <h3 style="font-size: 1.125rem; font-weight: 600; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
                ...
                <span style="margin-left: auto; display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap; justify-content: flex-end;">
                    ${dateStr ? `<span style="font-size: 0.72rem; color: var(--text-secondary); font-weight: 400;">Utolsó értékelés: ${dateStr}</span>` : ''}
                    ...
                    <button id="btn-reevaluate-ai" class="btn btn-secondary" style="font-size: 0.72rem; padding: 0.2rem 0.65rem; border-radius: 1rem; display: flex; align-items: center; gap: 0.3rem;">
                        <i data-lucide="refresh-cw" style="width: 0.75rem;"></i> Új értékelés
                    </button>
                </span>
            </h3>
            ...
            <div style="display: grid; grid-template-columns: auto 1fr; gap: 2rem; align-items: start; margin-bottom: 1.5rem;">
                ...
                <div>
                    <div style="background: ${rec.bg}; border-left: 3px solid ${rec.c}; border-radius: var(--radius-md); padding: 0.75rem 1rem; margin-bottom: 1rem; display: flex; align-items: center; gap: 0.75rem;">
                        <strong style="color: ${rec.c}; white-space: nowrap;">Ajánlás:</strong>
            ...
            <div style="border-top: 1px solid var(--border-color); padding-top: 1.25rem;">
                <div style="font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-secondary); margin-bottom: 0.75rem;">Kérdésenkénti összefoglalók</div>
                <div style="display: flex; flex-direction: column; gap: 0.4rem;">
                    ${result.perAnswer.map((pa, i) => {
                        ...
                        <div style="display: flex; align-items: baseline; gap: 0.75rem; font-size: 0.78rem; padding: 0.45rem 0.75rem; border-left: 3px solid ${pc}; background: ${pc}08; border-radius: 2px;">
                            <span style="font-weight: 700; color: ${pc}; min-width: 2.5rem;">${pa.score}p</span>
    ...
    return `
        <div class="card mb-6" style="padding: 2rem;" id="ai-score-card">
            ...
            <div style="padding: 1rem 1.25rem; background: ${noKey ? 'rgba(59,130,246,0.06)' : 'rgba(239,68,68,0.06)'}; border: 1px solid ${noKey ? 'rgba(59,130,246,0.2)' : 'rgba(239,68,68,0.2)'}; border-radius: var(--radius-md); display: flex; gap: 1rem; align-items: flex-start;">
                ...
                <div style="flex: 1;">
                    <div style="font-weight: 600; margin-bottom: 0.25rem;">${noKey ? 'Gemini API kulcs nincs beállítva' : 'AI kiértékelési hiba'}</div>
                    <div style="font-size: 0.875rem; color: var(--text-secondary);">
                        ${noKey
                            ? 'Az AI kiértékeléshez meg kell adni a Gemini API kulcsot az Admin panelben.'
                            : `Hiba: ${errorMsg}`}
                    </div>
                </div>
                ${noKey
                    ? `<button class="btn btn-secondary" onclick="window.navigateTo('adminPanel')" style="font-size: 0.8rem; white-space: nowrap; flex-shrink: 0;"><i data-lucide="settings"></i> Admin panel</button>`
                    : `<button class="btn btn-secondary" id="btn-retry-ai" data-id="${interviewId}" style="font-size: 0.8rem; white-space: nowrap; flex-shrink: 0;"><i data-lucide="refresh-cw"></i> Újra</button>`}
            </div>
        </div>`;
}
...
function renderAnswerFeedback(q, ansData, aiPerAnswer, isSelf) {
        ...
        return `
            <div style="margin-top: 0.75rem;">
                ${ansData.text
                    ...
                    : `<p style="color: var(--text-secondary); font-style: italic; margin-bottom: 0.75rem;">Nem adott meg választ.</p>`}
                ${pa ? `
    ...
    // HR-led
    let badge = '';
    if      (ansData.value === 'yes') badge = `<span style="background:rgba(16,185,129,0.1);color:var(--success);padding:0.25rem 0.75rem;border-radius:1rem;font-size:0.75rem;font-weight:600;">Igen</span>`;
    else if (ansData.value === 'no')  badge = `<span style="background:rgba(239,68,68,0.1);color:var(--danger);padding:0.25rem 0.75rem;border-radius:1rem;font-size:0.75rem;font-weight:600;">Nem</span>`;
    ...
    else badge = `<span style="background:var(--border-color);color:var(--text-secondary);padding:0.25rem 0.75rem;border-radius:1rem;font-size:0.75rem;font-weight:600;">Nincs megadva</span>`;

    const noteHtml = ansData.note
        ? `<div style="background:var(--bg-primary);padding:1rem;border-radius:var(--radius-md);margin-top:0.75rem;font-size:0.875rem;"><span style="color:var(--text-secondary);font-weight:500;display:block;margin-bottom:0.25rem;">HR Megjegyzés:</span>${ansData.note}</div>`
        : '';
...
function renderExitDisplay(container, interview) {
    ...
    const reasonLabels = {
        better_offer: 'Jobb ajánlat máshol', career_growth: 'Karrier / fejlődési lehetőség',
        management: 'Kollégiális / vezetői problémák', salary: 'Bérezés',
        worklife: 'Munka–magánélet egyensúly', relocation: 'Költözés / személyes okok', other: 'Egyéb'
    };
    const recMap = { yes: '✅ Igen, ajánlaná', maybe: '🤔 Talán', no: '❌ Nem ajánlaná' };
    ...
    container.innerHTML = `
        <div class="mb-6">
            ...
            <div class="flex justify-between items-start">
                <div>
                    <h2 style="font-size: 1.5rem; font-weight: 600;">Kilépő kérdőív: ${interview.candidateName}</h2>
                    <div class="flex gap-4 mt-2" style="font-size: 0.875rem; color: var(--text-secondary); flex-wrap: wrap;">
                        ...
                        ${interview.issuedByName ? `<span><i data-lucide="send" style="width: 1rem;"></i> Küldte: <strong>${interview.issuedByName}</strong></span>` : ''}
                    </div>
                </div>
                <div style="background:rgba(245,158,11,0.1);border:1px solid var(--warning);color:var(--warning);border-radius:var(--radius-round);padding:0.4rem 1rem;font-size:0.875rem;font-weight:600;display:flex;align-items:center;gap:0.5rem;">
                    <i data-lucide="log-out" style="width:1rem;"></i> Kilépő
                </div>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
            <div class="card">
                <div style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:var(--text-secondary);margin-bottom:0.75rem;">Alapadatok</div>
                ${ed.dept   ? `<div class="mb-2"><span style="color:var(--text-secondary);font-size:0.8rem;">Osztály:</span><div style="font-weight:500;">${ed.dept}</div></div>` : ''}
                ${ed.reason ? `<div class="mb-2"><span style="color:var(--text-secondary);font-size:0.8rem;">Kilépés oka:</span><div style="font-weight:500;">${reasonLabels[ed.reason]||ed.reason}</div></div>` : ''}
                ${ed.recommend ? `<div><span style="color:var(--text-secondary);font-size:0.8rem;">Ajánlás:</span><div style="font-weight:500;">${recMap[ed.recommend]||ed.recommend}</div></div>` : ''}
            </div>
            <div class="card">
                <div style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:var(--text-secondary);margin-bottom:0.75rem;">Mi tetszett legjobban</div>
                <p style="font-size:0.9375rem;line-height:1.6;">${ed.liked||'<em style="color:var(--text-secondary)">Nem adott meg választ.</em>'}</p>
            </div>
        </div>
        ${ed.improve ? `<div class="card mb-6"><div style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:var(--text-secondary);margin-bottom:0.75rem;">Fejlesztési javaslatok</div><p style="font-size:0.9375rem;line-height:1.6;">${ed.improve}</p></div>` : ''}
        ${ed.other   ? `<div class="card mb-6"><div style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:var(--text-secondary);margin-bottom:0.75rem;">Egyéb visszajelzés</div><p style="font-size:0.9375rem;line-height:1.6;">${ed.other}</p></div>` : ''}
    ...
    document.getElementById('btn-save-eval')?.addEventListener('click', () => {
        ...
        setTimeout(() => { btn.innerHTML = `<i data-lucide="save"></i> Mentés`; btn.classList.replace('btn-success','btn-primary'); lucide.createIcons(); }, 2000);
    });
}
