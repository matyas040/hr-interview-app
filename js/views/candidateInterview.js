    ...
    if (!role || role.questions.length === 0) {
        container.innerHTML = `
            <div class="card" style="max-width: 600px; margin: 4rem auto; text-align: center;">
                ...
                <h2 style="font-size: 1.5rem; font-weight: 600;" class="mb-2">Érvénytelen link</h2>
                <p style="color: var(--text-secondary);">A megadott interjú link nem található vagy a munkakörhöz nem tartoznak kérdések.</p>
            </div>
        `;
        lucide.createIcons();
        return;
            ...
            container.innerHTML = `
                <div style="max-width: 640px; margin: 0 auto; padding-top: 2rem;">
                    <div style="text-align: center; margin-bottom: 2.5rem;">
                        ...
                        <h2 style="font-size: 1.75rem; font-weight: 600;">Üdvözlünk, ${candidateName}!</h2>
                        <p style="color: var(--text-secondary); margin-top: 0.5rem;">Munkakör: <strong style="color: var(--text-primary);">${role.title}</strong></p>
                        <p style="color: var(--text-secondary); font-size: 0.875rem; margin-top: 0.25rem;">${totalQuestions} kérdés vár rád${role.pdfName ? ' · 📎 Olvasnivaló dokumentum csatolva' : ''}</p>
                    </div>

                    <div class="card" style="padding: 2rem;">
                        <h3 style="font-size: 1.125rem; font-weight: 600; margin-bottom: 1.5rem; display: flex; align-items: center; gap: 0.5rem;">
                            <i data-lucide="user" style="width: 1.25rem; color: var(--accent);"></i> Személyes adatok
                        </h3>
                        ...
                        <div class="mt-6 flex justify-end">
                            <button class="btn btn-primary" id="btn-start-questions" style="padding: 0.75rem 2rem; font-size: 1rem;">
                                ${role.pdfName ? 'Tovább a dokumentumhoz' : 'Tovább a kérdésekhez'} <i data-lucide="arrow-right"></i>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            container.innerHTML = `
                <div style="max-width: 900px; margin: 0 auto; padding-top: 2rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem;">
                        <div>
                            <h2 style="font-size: 1.5rem; font-weight: 600;">Olvasd el a dokumentumot</h2>
                    ...
                    <div class="card" style="padding: 1.25rem 1.5rem;">
                        <label style="display: flex; align-items: center; gap: 0.75rem; cursor: pointer;">
                            ...
                            <span style="font-weight: 500;">Elolvastam és megértettem a dokumentumot.</span>
                        </label>
                        <div class="mt-4 flex justify-end">
                            <button class="btn btn-primary" id="btn-continue-after-pdf" disabled style="padding: 0.75rem 2rem; font-size: 1rem; opacity: 0.5;">
                                Tovább a kérdésekhez <i data-lucide="arrow-right"></i>
                            </button>
                        </div>
                    </div>
                </div>
        ...
        container.innerHTML = `
            <div style="max-width: 800px; margin: 0 auto; padding-top: 2rem;">
                ...
                <div style="text-align: center; margin-bottom: 2rem;">
                    ...
                    <p style="color: var(--text-secondary); font-size: 0.875rem;">${role.title}</p>
                </div>
                ...
                <div class="mb-6">
                    <div style="display: flex; justify-content: space-between; font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 0.5rem;">
                        ...
                        <span>${Math.round(progressPercent)}% kitöltve</span>
                    </div>
                ...
                <div class="card mb-6" style="padding: 2.5rem;">
                    <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem; margin-bottom: 1.5rem;">
                        ...
                        ${question.answerType === 'short'
                            ? `<span style="flex-shrink: 0; background: rgba(245,158,11,0.12); color: #d97706; border: 1px solid rgba(245,158,11,0.3); border-radius: 1rem; padding: 0.25rem 0.75rem; font-size: 0.75rem; font-weight: 700; white-space: nowrap; margin-top: 0.25rem;">⚡ Rövid válasz</span>`
                            : `<span style="flex-shrink: 0; background: rgba(59,130,246,0.1); color: var(--accent); border: 1px solid rgba(59,130,246,0.25); border-radius: 1rem; padding: 0.25rem 0.75rem; font-size: 0.75rem; font-weight: 700; white-space: nowrap; margin-top: 0.25rem;">📝 Részletes válasz</span>`
                        }
                    </div>
                    <div class="form-group mb-0">
                        <label class="form-label" style="display: flex; align-items: center; gap: 0.5rem;">
                            <i data-lucide="edit-3" style="width: 1rem; height: 1rem;"></i> Válaszod
                            ${question.answerType === 'short'
                                ? `<span style="font-weight: 400; color: var(--text-secondary); font-size: 0.8rem;">(1-2 mondat elegendő)</span>`
                                : `<span style="font-weight: 400; color: var(--text-secondary); font-size: 0.8rem;">(fejtsd ki részletesen)</span>`
                            }
                        </label>
                        <textarea id="cand-ans" class="form-textarea"
                            placeholder="${question.answerType === 'short' ? 'Írj egy rövid, tömör választ...' : 'Kérlek írd le részletesen a tapasztalataidat, gondolataidat...'}"
